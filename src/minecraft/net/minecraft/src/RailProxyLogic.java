...
class RailProxyLogic {
	...
	final BlockProxyRail rail;

	public RailProxyLogic(BlockProxyRail blockRail2, World world2, int i3, int i4, int i5) {
		this.rail = blockRail2;
	...
	private void func_785_b() {
		for(int i1 = 0; i1 < this.connectedTracks.size(); ++i1) {
			RailProxyLogic railLogic2 = this.getMinecartTrackLogic((ChunkPosition)this.connectedTracks.get(i1));
	...
	private RailProxyLogic getMinecartTrackLogic(ChunkPosition chunkPosition1) {
		return BlockRail.isRailBlockAt(this.worldObj, chunkPosition1.x, chunkPosition1.y, chunkPosition1.z) ? new RailProxyLogic(this.rail, this.worldObj, chunkPosition1.x, chunkPosition1.y, chunkPosition1.z) : (BlockRail.isRailBlockAt(this.worldObj, chunkPosition1.x, chunkPosition1.y + 1, chunkPosition1.z) ? new RailProxyLogic(this.rail, this.worldObj, chunkPosition1.x, chunkPosition1.y + 1, chunkPosition1.z) : (BlockRail.isRailBlockAt(this.worldObj, chunkPosition1.x, chunkPosition1.y - 1, chunkPosition1.z) ? new RailProxyLogic(this.rail, this.worldObj, chunkPosition1.x, chunkPosition1.y - 1, chunkPosition1.z) : null));
	}

	private boolean isConnectedTo(RailProxyLogic railLogic1) {
	...
	private boolean handleKeyPress(RailProxyLogic railLogic1) {
	...
	private void func_788_d(RailProxyLogic railLogic1) {
	...
	private boolean func_786_c(int i1, int i2, int i3) {
		RailProxyLogic railLogic4 = this.getMinecartTrackLogic(new ChunkPosition(i1, i2, i3));
		if(railLogic4 == null) {
			return false;
		} else {
	...
	public void func_792_a(boolean z1, boolean z2) {
		...
		if(z2 || this.worldObj.getBlockMetadata(this.trackX, this.trackY, this.trackZ) != i8) {
			...
			for(int i9 = 0; i9 < this.connectedTracks.size(); ++i9) {
				RailProxyLogic railLogic10 = this.getMinecartTrackLogic((ChunkPosition)this.connectedTracks.get(i9));
				if(railLogic10 != null) {
	...
	static int getNAdjacentTracks(RailProxyLogic railLogic0) {
